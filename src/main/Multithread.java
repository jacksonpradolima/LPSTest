...
public class Multithread {
    ...
    private static synchronized void initialize() {
        ...
        try {
            ...
            for (final String instance : Settings.INSTANCES) {
                for (final MetaheuristicType algorithm : Settings.ALGORITHMS) {
                    for (final int populationSize : Settings.POPULATION_SIZE) {
                        for (final int generations : Settings.GENERATIONS) {
                            for (final double crossoverProbability : Settings.CROSSOVER_PROBABILITY) {
                                for (final double mutationProbability : Settings.MUTATION_PROBABILITY) {
                                    for (final String crossoverOperator : Settings.CROSSOVER_OPERATORS) {
                                        for (final String mutationOperator : Settings.MUTATION_OPERATORS) {
                                            for (final String selectionOperator : Settings.SELECTION_OPERATORS) {
                                                createThread(instance, algorithm, populationSize, generations, crossoverProbability, mutationProbability, crossoverOperator, mutationOperator, Settings.EXECUTIONS, selectionOperator);
                                            }
                                        }
                                    }
                                }
    ...
    private static synchronized void createThread(final String context) {
        //instance_algorithms_populationSize_generations_crossoverProbability_mutationProbability_crossoverOperator_mutationOperator_executions_selectionOperator
        String[] split = context.split("_");
        final String instance = String.valueOf(split[1]);
        final MetaheuristicType algorithm = MetaheuristicType.valueOf(split[2]);
        ...
        final String selectionOperator = String.valueOf(split[10]);

        createThread(instance, algorithm, populationSize, generations, crossoverProbability, mutationProbability, crossoverOperator, mutationOperator, executions, selectionOperator);
    }

    private static synchronized void createThread(final String instance, final MetaheuristicType algorithm, final int populationSize, final int generations, final double crossoverProbability, final double mutationProbability, final String crossoverOperator, final String mutationOperator, final int executions, final String selectionOperator) {

        final String context = Parameters.generateAlgorithmId(algorithm, populationSize, generations, crossoverProbability, mutationProbability, crossoverOperator, mutationOperator, executions, selectionOperator);
    ...
    private static String getExperimentClassName(final MetaheuristicType algorithm) {
